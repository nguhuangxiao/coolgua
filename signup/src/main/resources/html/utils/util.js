...
function autoLogin(app, e, that, link){
  //检查用户是否授权
  if (!e.detail.userInfo) {
    wx.showModal({
      title: '提示',
      content: '小瓜报名需要使用您的基本资料进行登录，为确保正常使用请允许！',
  ...
  wx.login({
    success: res => {
      //获取邀请人openId
      var inviteOpenId = that.data.inviteOpenId ? that.data.inviteOpenId : ""
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          var param = { openId: res.data.openid }
          goPage(link, param)
        }
      })
    }
