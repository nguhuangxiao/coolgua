...
import { Input, Select, DatePicker, Upload, Button, Icon, message, Spin } from 'antd'
...
import { getCountryData, getProvinceData, getCityData, saveActivityData, getActivityById } from 'api/activity_add'
...
class ActivityAdd extends Component{
      ...
      <div className="wrapper">
        ...
        <div className="containner main-flex">
          ...
          { this.state.isShow === false ?
            ...
            <div className="side-main entrance-main">
              <div className="activity-main">
                <div className="setting-item">
                  <div className="item">
                    <label>活动主题<span>*</span></label>
                    <Input className="form-input" name="name" id="name" placeholder="活动主题字数不能超过20个汉字" value={this.state.name} onChange={this.handleChange.bind(this)}/>
                  </div>
                  <div className="item">
                  ...
                  <div className="item">
                    ...
                    <div className="more-address">
                      <span>详细地址</span><Input className="form-input" id="address" value={this.state.address} name="address" onChange={this.handleChange.bind(this)}/>
                    </div>
                  </div>
                  ...
                  <div className="item">
                    <label>主色调</label>
                    <input type="text" name="color" readOnly style={{background: this.state.color}} id="color" value={this.state.color} ref="color" onChange={this.handleChange.bind(this)} className="ant-input iColorPicker color-ant" />
                    <a href="javascript:void(null)" id="icp_color" onClick={this.chooseColor.bind(this)}>
                      <img src={colorPicture} align="absmiddle" />
                    </a>
                  </div>
                  <div className="item">
                    <Field layout={this.state.layout}/>
                  </div>
  ...
  componentDidMount() {
    const listItem = getActivityById(this.props.match.params.id)
    listItem.then((res) => {
      if(res.code === 200){
        var currentData = res.event
        const time = currentData.eventTime.split('~')
        this.setState({
          isShow: true,
          id: currentData.id,
          ...
          endOpen: false,
          registerStartTime: moment(currentData.registerStartTime),
          registerEndTime: moment(currentData.registerEndTime),
          registerEndOpen: false,
